...
const followUser = async (req, res) => {
  ...
  try {
    ...
    //save the update current user, user to follow
...
const deleteUserFromRequest = async (req, res) => {
  try {
    ...
    //check both user is exist in database or not
  ...
  try {
    ...
    const userToFollowBack = await User.find({
      _id: {
        ...
        $nin: loggedInUser.following, // exclude users the logged in user already follow back
      },
